...
**Migrations:** `backend/alembic/versions/` (`0001` … `0005`)

This document describes the **current** schema after the Node/Mongo → Python/Postgres rewrite and the question-bank redesign. Field names below are **snake_case** (DB / ORM). The API exposes camelCase via Pydantic aliases.
...
- Writable: `question_links` → `ExamTemplateQuestion` with `cascade="all, delete-orphan"` (delete template ⇒ delete **links only**).
- Read-only convenience: `questions` secondary relationship is **`viewonly=True`** so SQLAlchemy does not double-write the join table or cascade-delete bank questions.
...
Same question may appear in many templates.
...
On create, the service **pre-creates one `answer_records` row per question** in template order, copying `order_index`. That freezes the sitting composition.
...
**Results path:** join `answer_records` → `questions` (including archived). Do **not** re-read `exam_template_questions` for finished sessions.
...
| `crtEnabled`           | bool             |
| `dailyXpGoal`          | int              |
| `audioSfxEnabled`      | bool (flag)      |
| `notificationsEnabled` | bool (flag)      |
...
## 5. Domain invariants (exam / bank)

These are product rules encoded in services + schema, not optional polish:

1. **Shared mutable bank.** Composition is only `exam_template_questions`. Never store a private copy of option text on the template.
2. **Session freezes composition** via pre-created `answer_records.order_index`.
3. **Soft-delete questions** (`archived_at`) so history survives.
4. **Freeze answer keys** after first answer (409 on options/correct_answer change).
5. **Built-in exams:** identify by `seed_key`, version with `seed_version`, update **in place** so `exam_template_id` on past sessions stays valid.
...
A raw SQL `DELETE FROM decks` without ORM might SET NULL instead. Prefer going through the service/ORM path; document UI accordingly.
...
- Cards: `user_id`, `deck_id`, `srs_next_review`
- Questions: `user_id`, `exam_type`, `part`, `passage_group`, `archived_at`
- Exam templates: `user_id`, `exam_type`, `is_public`, `seed_key` (unique)
- Join: `exam_template_id`, `question_id` + unique pair
- Sessions: `user_id`, `exam_template_id`, `status`
- Answer records: `session_id`, `question_id`
...
| ------------------------- | --------------------------------------------------------------------- |
| JWT auth tokens           | Client storage / signed JWT, not a sessions table                     |
| Unfiled card count        | Aggregated client-side or via card query (`deck_id IS NULL`)          |
| Live exam timer deadline  | Client deadline from `time_limit_minutes` at start (not a DB column)  |
| Full passage entity model | Flat `passage` + `passage_group` on `questions` only                  |
| Media blobs               | URLs on cards (`image_url`); media router may store files outside SQL |
...
| Deck/card services                    | `backend/app/services/` (card position, cascade delete)        |
| Exam session create / answer / finish | `backend/app/services/` + `routers/exams.py`                   |
| Built-in exam seed                    | app lifespan / seed module (keys on `exam_templates.seed_key`) |
| Pydantic I/O (camelCase)              | `backend/app/schemas/`                                         |

---

_Last aligned with models under `backend/app/models/` as of migrations through `0005_user_settings`._
