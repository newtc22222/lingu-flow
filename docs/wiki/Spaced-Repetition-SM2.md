...
- $I_{prev}$: Previous interval in days
- $EF_{prev}$: Previous ease factor
- $n_{prev}$: Previous consecutive success count (repetitions)
...
If $q \ge 3$ (User rated **Hard**, **Good**, or **Easy**):
- If $n_{prev} == 0 \implies I_{next} = 1$ day
- If $n_{prev} == 1 \implies I_{next} = 6$ days
- If $n_{prev} \ge 2 \implies I_{next} = \lceil I_{prev} \times EF_{prev} \rceil$
- $n_{next} = n_{prev} + 1$
...
$$EF_{next} = EF_{prev} + \left(0.1 - (5 - q) \times (0.08 + (5 - q) \times 0.02)\right)$$

> [!IMPORTANT]
> **Ease Factor Lower Bound**:
> The Ease Factor is bounded by a strict minimum floor to prevent cards from becoming impossible to schedule:
> $$EF_{next} = \max(1.3, EF_{next})$$

### 3. Scheduling Next Review Timestamp

The next review date is calculated by adding $I_{next}$ days to the current UTC timestamp:

$$\text{NextReviewDate} = \text{CurrentTime} + \text{timedelta}(\text{days} = I_{next})$$
...
def calculate_sm2(
    ...
    quality = SCORE_TO_QUALITY.get(score, 0)
