...
|---------|-------:|---------|--------|
| A1 | https://github.com/newtc22222/lingu-flow/issues/44 | F-01 | Production JWT guard reads wrong `ENVIRONMENT` source |
| A2 | https://github.com/newtc22222/lingu-flow/issues/45 | F-02, F-03 | Private exam IDOR + session details leak answer keys |
| A3 | https://github.com/newtc22222/lingu-flow/issues/46 | F-09 | Remove unused SSE `/api/events` (JWT in query string) |
...
- **Add:** temp `.env` (or `Settings` with `_env_file=...`) where `ENVIRONMENT=production` and JWT is missing/default → **raises**.
- **Add:** production + strong random secret → **accepts**.
- Development + default secret → still works.
...
3. **Answer key rule:**
   - `session.status == "completed"` **and** owner → include `correctAnswer` / explanations (ExamResults needs this).
   - `in-progress` (or any non-completed) → questions without answer keys even for the owner.
...
3. Implement A1 → A2 → A3 (order reduces risk of mixing concerns).
...
On Windows PowerShell that path is correct when venv exists. If venv layout differs, use the project’s usual pytest entry.
...
- **Backend-first.** Match existing router → service → schema layering.
...
(Adjust test filenames if you chose different module names.)

Grep sanity:

```bash
# should find no os.getenv ENVIRONMENT in JWT validator path after fix
# should find no include_router(events
# should find _readable_template_or_404 (or equivalent name) used by create_session and template GETs
```
