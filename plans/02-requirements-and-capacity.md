...
Already present (do not redesign wholesale):

```
/api/auth/*          login, register, guest, google, forgot-password
/api/cards/*         CRUD, study, review
/api/decks/*         CRUD, cards, reorder
/api/exams/*         templates, composition, sessions, answers, finish, details
/api/questions/*     bank CRUD, tags, parts
/api/dashboard/*     progress
/api/settings/*      user prefs
/api/media/*         presign
/api/health          liveness
/api/events          SSE (candidate for removal)
```

Contract rules:

- CamelCase JSON via Pydantic aliases.
- Prefer `id` over Mongo-era `_id` in new FE code.
- Silent `?? fallback` on shape mismatch is a known failure mode — prefer hard errors in new paths.

---

## Success metrics for the improvement program

| Metric | Baseline (approx) | Target |
|--------|-------------------|--------|
| Critical open findings (F-01–F-04) | 4 open | 0 open |
| Security probes in CI | 0 | ≥ 6 cases covering F-01–F-03 |
| Transaction ownership | dual commit | single owner (`get_db`) |
| Cascade tests on Postgres | 0 | ≥ 5 critical paths |
| Doc/skill accuracy | known wrong | agents don’t get misled |
| Dashboard query pattern | full table load | aggregates |
| Phase 2 start | blocked by trust | unblocked after Horizon A |

---

## Next

→ Deep dives: [03-security-hardening.md](./03-security-hardening.md), [04-data-integrity-transactions.md](./04-data-integrity-transactions.md).
