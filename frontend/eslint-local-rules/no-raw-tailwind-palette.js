...
const PREFIXES = [
  'bg', 'text', 'border', 'ring', 'from', 'via', 'to', 'divide', 'outline',
  'fill', 'stroke', 'shadow', 'accent', 'caret', 'decoration',
]
const CLASS_RE = new RegExp(
  `^(?:[a-z]+:)?(${PREFIXES.join('|')})-(${PALETTE.join('|')})-[0-9]+`,
)
        ...
        VExpressionContainer(node) {
          ...
          if (expr.type === 'ObjectExpression') {
            for (const prop of expr.properties) {
              const key = prop.key
              if (key?.type === 'Literal' && typeof key.value === 'string') literals.push(key.value)
