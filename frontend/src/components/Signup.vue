...
const handleSignup = async () => {
  ...
  try {
    ...
    if (!res.ok) {
      errorMsg.value = data.error || 'Signup failed';
      return;
    }
    ...
    errorMsg.value = 'An error occurred during signup.';
  } finally {
...
<template>
  <div class="flex items-center justify-center h-full bg-slate-900 text-slate-100 p-4 overflow-y-auto">
    <div class="bg-slate-800 p-8 rounded-2xl border border-slate-700 w-full max-w-md shadow-2xl my-4">
      <h2 class="text-3xl font-bold text-center text-emerald-400 mb-8">Create Account</h2>
      ...
      <div class="mt-6 text-center text-slate-400 text-sm">
        ...
        <button @click="emit('go-to-login')" class="text-emerald-400 hover:underline">Log in</button>
      </div>
    </div>
  </div>
</template>
